...
/**
 * Calculates a penalty multiplier based on the number of times a document has been rejected.
 * @private
 * @param {string} documentId - The ID of the uploaded document.
 * @returns {Promise<number>} The history multiplier.
 */
const _calculateHistoryMultiplier = async (documentId) => {
  if (!documentId) return 1.0;
  ...
  if (rejectionCount >= 3) {
    return scoringWeights.historyMultiplier[3];
  }
...
export const calculateVendorScore = async (vendorId) => {
  ...
  // Get wage verification data
  const wageVerifications = await prisma.wageVerification.findMany({
    where: { vendorId: vendorId },
    orderBy: { createdAt: 'desc' },
    take: 1 // Get the most recent wage verification
  });
  ...
  // Add wage verification score if available
  if (wageVerifications.length > 0) {
    const latestWageVerification = wageVerifications[0];
    const wageScore = latestWageVerification.riskScore || 0;
    const wageWeight = scoringWeights.pillars.WAGES_OVERTIME || 1.2;
    
    totalWeightedScore += wageScore * wageWeight;
    totalPillarWeight += wageWeight;
    validDocumentsCount++;
  }
